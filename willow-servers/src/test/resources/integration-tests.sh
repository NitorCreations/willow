...
mvn $REPO -f target/test-classes/deploy-pom.xml clean install com.nitorcreations:willow-maven-plugin:$WILLOW_VERSION:upload
...
mvn $REPO -f target/test-classes/deploy-pom.xml com.nitorcreations:willow-maven-plugin:$WILLOW_VERSION:properties
TEST_RETURN=$(($TEST_RETURN + $?))
